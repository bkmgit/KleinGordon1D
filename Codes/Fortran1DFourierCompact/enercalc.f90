	...
	COMPLEX(KIND=8), DIMENSION(1:Nx),INTENT(OUT)	:: temp1,temp2
	...
	INTEGER(kind=4), INTENT(IN)            ::  ntable,nwork
        ...
        REAL(kind=8)                           ::  scale
        REAL(kind=8), DIMENSION(1:ntable)      ::  table
	...
	END DO
	!$OMP END PARALLEL DO	
        CALL ZFFT(isignb,Nx,scale,temp1,incx,temp2,incy,table,ntable,work,nwork)
        enstr=0.0d0
	!!$OMP PARALLEL DO PRIVATE(i) SCHEDULE(static)
	DO i=1,Nx
