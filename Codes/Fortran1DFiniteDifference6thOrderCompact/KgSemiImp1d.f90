	...
	! using a fourt order implicit-explicit time stepping scheme.
	!
	...
	INTEGER(kind=4)				::  count_rate, plotnum
        INTEGER(kind=4)                         ::  ip,ipp,ippp,im,imm,immm,iter
		...
		DO i=1,Nx
			...
			rhs(i)=Es*(u(i)**3) &
                              ...
                              +( (1.0d0/90.0d0)*u(immm) &
                                       ...
                                       +(3.0d0/2.0d0)*u(ip) &
                                       -(3.0d0/20.0d0)*u(ipp) &
                                       +(1.0d0/90.0d0)*u(ippp))/(dx*dx) &
                              -u(i) &
                              +(2.0d0/(4.0d0*3.0d0*2.0d0))* &
                               ( 3.0d0*u(i)*u(i)*(-2.0d0*u(i)+uold(i)) &
                                +6.0d0*u(i)*(-u(i))*(u(i)-uold(i)) &
                                -1.0d0*(-2.0d0*u(i)+uold(i)) &
                                +( (1.0d0/90.0d0)*(-2.0d0*u(immm)+uold(immm)) &
                                  -(3.0d0/20.0d0)*(-2.0d0*u(imm)+uold(imm)) &
                                  +(3.0d0/2.0d0)*(-2.0d0*u(im)+uold(im)) &
                                  -(49.0d0/18.0d0)*(-2.0d0*u(i)+uold(i)) &
                                  +(3.0d0/2.0d0)*(-2.0d0*u(ip)+uold(ip)) &
                                  -(3.0d0/20.0d0)*(-2.0d0*u(ipp)+uold(ipp)) &
                                  +(1.0d0/90.0d0)*(-2.0d0*u(ippp)+uold(ippp)))/(dx*dx) )  
                        r(i)=rhs(i)-u(i)*(1.0d0/(dt*dt) ) &
                                +(2.0d0/(4.0d0*3.0d0*2.0d0))*( &
                                  ( (1.0d0/90.0d0)*u(immm) &
                                   -(3.0d0/20.0d0)*u(imm) &
                                   +(3.0d0/2.0d0)*u(im) &
                                   ...
                                   +(1.0d0/90.0d0)*u(ippp) )/(dx*dx) &
                                   -u(i) &
                                 +(3.0d0*u(i)*u(i) +6.0d0*u(i)*(u(i)-uold(i)))*u(i)  )
                        p(i)=r(i)
                        unew(i)=u(i)
                        rsum=rsum+abs(r(i))
                        rr=rr+r(i)*r(i)
			...
			Mp(i)=p(i)*(1.0d0/(dt*dt)  ) &
                             -(2.0d0/(4.0d0*3.0d0*2.0d0))*( &
                                    ( (1.0d0/90.0d0)*p(immm) &
                                      ...
                                      +(3.0d0/2.0d0)*p(ip) &
                                      -(3.0d0/20.0d0)*p(ipp) &
                                      +(1.0d0/90.0d0)*p(ippp) )/(dx*dx) &
                                      -p(i) &
                                      +(3.0d0*u(i)*u(i)+6.0d0*u(i)*(u(i)-uold(i)))*p(i) )
		  END DO
		  !$OMP END PARALLEL DO
                  alpha=0.0d0
